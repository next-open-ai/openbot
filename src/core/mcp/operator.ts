...
import { stdioConfigKey } from "./config.js";
 ...
 */
export async function getMcpToolDefinitions(serverConfigs: McpServerConfigStdio[]): Promise<ToolDefinition[]> {
    const allTools: ToolDefinition[] = [];
    for (let i = 0; i < serverConfigs.length; i++) {
        const config = serverConfigs[i];
        const key = stdioConfigKey(config);
        ...
        if (!client) {
            ...
            try {
                ...
                console.warn(`[mcp] 连接失败 (${config.command}):`, err instanceof Error ? err.message : err);
                continue;
            }
        }
        if (!client.isConnected) {
            ...
            try {
                ...
                console.warn(`[mcp] 重连失败 (${config.command}):`, err instanceof Error ? err.message : err);
                continue;
            }
        }
        try {
            ...
            console.warn(`[mcp] list_tools 失败 (${config.command}):`, err instanceof Error ? err.message : err);
        }
    }
    return allTools;
}
