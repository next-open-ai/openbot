...
A：未做 Apple 公证时，系统可能加「隔离」属性。在终端执行：`xattr -cr /Applications/OpenBot.app`（路径按实际修改），然后重新打开。
...
- **npm 全局安装**：若因 `node-llama-cpp` 等原生依赖安装失败，可使用 `npm install -g @next-open-ai/openbot --ignore-scripts` 跳过可选原生模块，对 CLI/Gateway/Desktop 常规使用无影响；长记忆需在设置中配置在线 RAG 或具备本地编译环境。
...
**Q：通道使用的智能体可以选 Coze、OpenBot 或 OpenCode 吗？**  
A：可以。在「设置 → 智能体」中配置执行方式为 coze、openclawx 或 opencode 的智能体，并在通道配置或 defaultAgentId 中指定该智能体即可。

**Q：多节点协作如何配置？**  
A：在另一台机器启动 OpenBot Gateway，本机在智能体配置中选择执行方式为 OpenBot，填写对方 baseUrl（及可选 apiKey）即可。详见 [代理模式与多节点协作](../features/proxy-mode.md)。
