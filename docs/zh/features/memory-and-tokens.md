...
**结论**：OpenBot 是「工具/会话事件写向量库」；Moltbot 是「先写 Markdown 文件，再由 sync 把文件 chunk+embed 进 SQLite」，没有 `save_experience` 这种直接写向量的工具。
...
**结论**：OpenBot 已与 Moltbot 对齐为「按需查记忆，仅工具调用时带入」→ 同一轮数下 token 消耗取决于 Agent 是否、以及多频繁地调用 memory_recall。
...
**总体**：OpenBot 已采用与 Moltbot 一致的「按需 memory_recall」策略，记忆只在 Agent 主动调用工具时进入对话，token 消耗与调用频率和返回条数相关。
