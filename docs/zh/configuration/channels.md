...
OpenBot 支持通过**通道**将 Agent 对接到飞书、钉钉、Telegram。通道在 Gateway 启动时根据配置注册：入站消息经统一格式进入 Agent，回复经该通道发回对应平台。
...
- **用法**：飞书开放平台创建自建应用、开通「机器人」与「接收消息」、事件订阅选 WebSocket；在 OpenBot 设置 → 通道 勾选「启用飞书」并填写 App ID、App Secret → 保存后重启 Gateway；在飞书内私聊或群聊 @ 机器人即可，回复以流式卡片更新。
...
- **用法**：钉钉开发者后台创建企业内部应用、添加机器人能力并选择 **Stream 模式**；在 OpenBot 设置 → 通道 启用钉钉并填写 Client ID、Client Secret → 保存后重启 Gateway；在钉钉内与机器人对话即可。
...
- **用法**：通过 [@BotFather](https://t.me/BotFather) 获取 Bot Token；在 OpenBot 设置 → 通道 启用 Telegram 并填写 Bot Token → 保存后重启 Gateway；在 Telegram 内与机器人对话即可。
