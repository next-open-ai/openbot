...
**术语说明**：本系统中「智能体」「助手」「专家」均指同一概念（Agent），可互换使用。界面或文档中出现的「XX 助手」「XX 专家」均为智能体的一种展示名称。
...
| **openclawx** | 代理至其他 OpenBot 实例，需配置 baseUrl、可选 apiKey |
...
- **OpenBot**：`execution: "openclawx"`，配置 **openclawx.baseUrl**、**openclawx.apiKey**（可选）。
...
- OpenBot：填写目标实例 baseUrl（如 `http://另一台机器:38080`）、可选 API Key。
