...
智能体除在本机运行（**local**）外，可配置为**代理模式**，将对话转发至 Coze、另一台 OpenBot 或 [OpenCode](https://opencode.ai/) 官方 Server，实现生态接入与多节点协作。
...
- 将智能体代理到另一台 OpenBot 实例：配置 **baseUrl**（如 `http://另一台机器:38080`）、可选 **apiKey**。
- 多台机器各跑一个 OpenBot Gateway，将部分智能体指向对方 baseUrl，即可实现分工、专机专用或负载均衡。
