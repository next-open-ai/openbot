...
Besides running locally (**local**), agents can use **proxy mode** to forward conversations to Coze, another OpenBot instance, or the [OpenCode](https://opencode.ai/) server.
...
| **openclawx** | Proxy to another OpenBot instance | Set baseUrl (e.g. `http://other-host:38080`), optional apiKey |
...
Point an agent at another OpenBot gateway via **baseUrl** and optional **apiKey**. Run gateways on multiple machines and assign agents to different nodes for load or specialization.
